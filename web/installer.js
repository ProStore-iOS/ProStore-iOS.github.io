...
const found = releases.find(r => {
  ...
  return false;
        ...
        let chosen = _chooseAssetFromRelease(_releaseInfo, recommended);
        if (!chosen) {
          // If the chosen release didn't have an ipa, try to fallback to the visible list to find any release with an .ipa
          const otherWithIpa = (visible.length ? visible : releases).find(r => _chooseAssetFromRelease(r, recommended));
          if (otherWithIpa) {
            _releaseInfo = otherWithIpa;
            chosen = _chooseAssetFromRelease(otherWithIpa, recommended);
          }
        }
